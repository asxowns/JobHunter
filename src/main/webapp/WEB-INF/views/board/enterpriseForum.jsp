		...
		<main>
			...
			<div class="page_navi">
				...
				<c:if test="${hasPrevious}">
					<a href="?page=${currentPage - 1}&size=${pageSize}&keyword=${keyword}&boardtype=2" class="prev">&#8249;</a>
				</c:if>
				...
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<c:choose>
						...
						<c:otherwise>
							<a href="?page=${i}&size=${pageSize}&keyword=${keyword}&boardtype=2">${i + 1}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<!-- 다음 페이지 링크 -->
				<c:if test="${hasNext}">
					<a href="?page=${currentPage + 1}&size=${pageSize}&keyword=${keyword}&boardtype=2" class="next">&#8250;</a>
				</c:if>
			</div>
